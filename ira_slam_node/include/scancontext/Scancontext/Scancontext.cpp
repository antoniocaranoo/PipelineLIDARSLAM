...
std::string SCManager::generateFilename()
{
    ...
    // Constructor with con configurable params
SCManager::SCManager(
    int num_rings,
    int num_sectors,
    double max_radius,
    ...
    PC_NUM_RING(num_rings),
    PC_NUM_SECTOR(num_sectors),
...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down, double timestamp)
{
    ...
    polarcontexts_timestamp_.push_back( timestamp );
    point_clouds_.push_back(std::make_shared<pcl::PointCloud<SCPointType>>(_scan_down));
...
SCManager::~SCManager() {
    if (file.is_open()) {
        file.close();
        std::cout << "File " << filename << " chiuso correttamente.\n";
    }
}
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( min_dist < SC_DIST_THRES )
    {
        ...
        if (file.is_open()) {
            file << "[Loop found] timestamp: "
                 << polarcontexts_timestamp_.back() << " s"
                 << " and " << nn_idx
                 << " (timestamp: " << polarcontexts_timestamp_[nn_idx] << " s)." << std::endl;
        }        
...
pcl::PointCloud<SCPointType>::Ptr SCManager::getPointCloud(int idx)
{
    if (idx >= 0 && idx < point_clouds_.size()) {
        return point_clouds_[idx];
    }
    return nullptr;
}

double SCManager::getTimestamp(int idx) const {
    if (idx >= 0 && idx < polarcontexts_timestamp_.size()) {
        return polarcontexts_timestamp_[idx];
    }
    return -1.0;
}

int SCManager::getNumFrames() const {
    return polarcontexts_timestamp_.size();  
}
